...
function MenteeNewPassword() {
    ...
    const[show,setShow]=useState('')
     const changeNewPassword = async(e) => {
       ...
       if (match) {
         const OTP = match[1];
         console.log(OTP)
         const res = await authInstance.post(`/mentee/reset-password/${OTP}`, {password});
  ...
  <div><p>{ show}</p></div>
    ...
    <Link to='/mentee-signin'>Singin</Link>
</div>
  </div>
  )
}

export default MenteeNewPassword